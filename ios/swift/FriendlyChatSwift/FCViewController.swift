...
UITextFieldDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
  ...
  func tableViewScrollToBottom(animated: Bool) {
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
      ...
      if numberOfRows > 0 {
        ...
        self.clientTable.scrollToRow(at: indexPath, at: .bottom, animated: animated)
      }
    }
  }
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    let msg = Message()
    msg.text = message[Constants.MessageFields.text]
    msg.senderName = message[Constants.MessageFields.name]
    msg.imageURL = message[Constants.MessageFields.imageURL]
    msg.photoURL = message[Constants.MessageFields.photoURL]
  ...
  func imagePickerController(_ picker: UIImagePickerController,
    ...
    if #available(iOS 8.0, *), let referenceURL = info[UIImagePickerControllerReferenceURL] {
      ...
      PHImageManager.default().requestImage(for: asset!, targetSize: CGSize(width: 1024, height: 1024), contentMode: .aspectFit, options: options, resultHandler: { (image, _) -> Void in
        ...
        self.sendImage(image: image, uid: uid, completed: { (error) in
          MBProgressHUD.hide(for: self.view, animated: true)
          if let error = error {
            self.showAlert(withTitle: "Error uploading", message: error.localizedDescription)
          }
        })
      })
    } else {
  ...
  func sendImage(image: UIImage!, uid: String!, completed: @escaping (_ error: Error?) -> ()) {
    ...
    self.storageRef.child(imagePath)
      .put(imageData!, metadata: metadata) { [weak self] (metadata, error) in
        ...
        FIRStorage.storage().reference(forURL: strongSelf.storageRef.child((metadata?.path)!).description).downloadURL(completion: { (url, error) in
          guard let url = url else { return }
          strongSelf.sendMessage(withData: [Constants.MessageFields.imageURL: url.absoluteString])
          completed(error)
        })
    }
  }
  ...
  @IBAction func signOut(_ sender: UIButton) {
    ...
    do {
      ...
      showAlert(withTitle: "Error signing out", message: signOutError.localizedDescription)
    }
  }
